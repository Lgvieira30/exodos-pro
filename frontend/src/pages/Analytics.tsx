...
const STATUS_BADGE: Record<string, { label: string; color: string; bg: string }> = {
  ...
  draft:     { label: 'Rascunho',   color: '#64748b', bg: 'rgba(100,116,139,0.12)' },
...
function KpiCard({ label, value, icon: Icon, gradient, glow, sub, subColor }: any) {
  return (
    <div style={{
      background: BG_CARD,
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: '14px' }}>
        ...
        {sub && (
          <span style={{
            ...
            color: subColor || 'rgba(255,255,255,0.4)',
            background: subColor ? `${subColor}18` : 'rgba(255,255,255,0.06)',
            border: `1px solid ${subColor ? subColor + '30' : 'rgba(255,255,255,0.08)'}`,
      ...
      <p style={{ fontSize: '22px', fontWeight: 800, color: '#fff', marginBottom: '4px', letterSpacing: '-0.02em' }}>{value}</p>
      <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.38)', fontWeight: 500 }}>{label}</p>
    </div>
  );
}
...
function ChartCard({ title, desc, children }: { title: string; desc: string; children: React.ReactNode }) {
  return (
    <div style={{
      ...
      <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)', marginBottom: '18px', lineHeight: 1.5 }}>{desc}</p>
      {children}
    </div>
  );
}
  ...
  const kpis = [
    ...
    },
    {
      ...
      subColor: summary?.cpa
        ? (Number(summary.cpa) <= 60 ? '#10b981' : Number(summary.cpa) <= 150 ? '#f59e0b' : '#ef4444')
        : undefined,
    },
    {
      ...
      subColor: summary?.roas
        ? (Number(summary.roas) >= 3 ? '#10b981' : Number(summary.roas) >= 2 ? '#f59e0b' : '#ef4444')
        : undefined,
    },
    {
      ...
      subColor: summary?.ctr
        ? (Number(summary.ctr) >= 2.5 ? '#10b981' : Number(summary.ctr) >= 1 ? '#f59e0b' : '#ef4444')
        : undefined,
    },
    {
      ...
      subColor: summary?.cpc
        ? (Number(summary.cpc) <= 5 ? '#10b981' : Number(summary.cpc) <= 15 ? '#f59e0b' : '#ef4444')
        : undefined,
    },
  ];
  ...
  return (
    <div className="page-pad" style={{ minHeight: '100vh', background: '#000', padding: '28px 32px' }}>
      ...
      <div style={{ marginBottom: '28px' }}>
        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', flexWrap: 'wrap', gap: '12px' }}>
          <div style={{ flex: 1 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '5px' }}>
              <h1 style={{ fontSize: '20px', fontWeight: 800, color: '#fff', letterSpacing: '-0.02em', margin: 0 }}>Analytics</h1>
              {fetching && (
                <span style={{
            ...
            <p style={{ fontSize: '12px', color: 'rgba(255,255,255,0.38)', margin: 0, fontWeight: 400 }}>
        ...
        {noData && (
          <div style={{ marginTop: '14px', display: 'inline-flex', alignItems: 'center', gap: '6px', fontSize: '11px', color: '#f59e0b', padding: '6px 14px', borderRadius: '8px', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.2)' }}>
      ...
      {campaigns.length > 0 && (
        <div style={{
          ...
          <div style={{
            ...
            <div>
              <p style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '2px' }}>Comparativo de Campanhas</p>
              <p style={{ fontSize: '11px', color: 'rgba(255,255,255,0.35)' }}>Métricas acumuladas por campanha no período</p>
            </div>
            <button
              ...
              style={{
                ...
                border: `1px solid ${CYAN}40`, background: `${CYAN}12`,
              ...
              <tbody>
                {campaigns.map((c) => {
                  ...
                  return (
                    ...
                    >
                      <td style={{ padding: '13px 18px', color: '#fff', fontWeight: 600, maxWidth: '200px', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{c.name}</td>
                      <td style={{ padding: '13px 18px', textAlign: 'right', color: 'rgba(255,255,255,0.45)', fontSize: '12px' }}>{PLATFORM_LABEL[c.platform] || c.platform}</td>
                      ...
                      <td style={{ padding: '13px 18px', textAlign: 'right', color: '#fff', fontWeight: 700 }}>R$ {Number(c.total_spend).toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '13px 18px', textAlign: 'right', color: '#fff', fontWeight: 500 }}>{Number(c.total_leads).toLocaleString('pt-BR')}</td>
                      <td style={{ padding: '13px 18px', textAlign: 'right', color: c.avg_cpa > 60 ? '#ef4444' : c.avg_cpa > 0 ? '#10b981' : 'rgba(255,255,255,0.3)', fontWeight: 700 }}>
                        {c.avg_cpa > 0 ? `R$ ${Number(c.avg_cpa).toFixed(0)}` : '—'}
                      </td>
                      <td style={{ padding: '13px 18px', textAlign: 'right', color: c.avg_roas >= 3 ? '#10b981' : c.avg_roas >= 2 ? '#f59e0b' : c.avg_roas > 0 ? '#ef4444' : 'rgba(255,255,255,0.3)', fontWeight: 700 }}>
                        {c.avg_roas > 0 ? `${Number(c.avg_roas).toFixed(1)}x` : '—'}
                      </td>
                      <td style={{ padding: '13px 18px', textAlign: 'right', color: c.avg_ctr >= 1.5 ? '#10b981' : c.avg_ctr >= 1 ? '#f59e0b' : c.avg_ctr > 0 ? '#ef4444' : 'rgba(255,255,255,0.3)', fontWeight: 700 }}>
                        {c.avg_ctr > 0 ? `${Number(c.avg_ctr).toFixed(2)}%` : '—'}
                      </td>
                      <td style={{ padding: '13px 18px', textAlign: 'right', color: 'rgba(255,255,255,0.55)', fontSize: '12px' }}>
      ...
      {campaigns.length > 0 && (
        <div style={{
          ...
          <p style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>Análise Individual por Campanha</p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '12px' }}>
            {campaigns.map((c) => {
              ...
              return (
                <div key={c.id} style={{ background: 'rgba(255,255,255,0.025)', border: `1px solid ${healthColor}22`, borderRadius: '14px', padding: '16px' }}>
                  ...
                  <button
                    ...
                    style={{
                      ...
                      border: '1px solid rgba(255,255,255,0.08)',
                      background: expandedCampaignId === c.id ? 'rgba(61,184,232,0.08)' : 'rgba(255,255,255,0.03)',
                      color: expandedCampaignId === c.id ? CYAN : 'rgba(255,255,255,0.4)',
                      ...
                      <div style={{ marginTop: '10px', borderTop: '1px solid rgba(255,255,255,0.06)', paddingTop: '10px' }}>
                        {deep.adSets?.length > 0 && (
                          <div style={{ marginBottom: '10px' }}>
                            <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.35)', marginBottom: '6px', letterSpacing: '0.05em' }}>CONJUNTOS ({deep.adSets.length})</p>
                        ...
                        {deep.ads?.length > 0 && (
                          <div>
                            <p style={{ fontSize: '10px', fontWeight: 700, color: 'rgba(255,255,255,0.35)', marginBottom: '6px', letterSpacing: '0.05em' }}>ANÚNCIOS ({deep.ads.length})</p>
                            {deep.ads.map((ad: any) => {
      ...
      <div style={{
        ...
        <p style={{ fontSize: '13px', fontWeight: 700, color: '#fff', marginBottom: '4px' }}>Destaques do Período</p>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: '12px' }} className="grid-insights">
          {[
            ...
            <div key={title} style={{ display: 'flex', alignItems: 'flex-start', gap: '12px', padding: '14px 16px', borderRadius: '12px', background: 'rgba(255,255,255,0.025)', border: '1px solid rgba(255,255,255,0.06)' }}>
